...
export async function retrieveRecentRounds(uid: string): Promise<{data: object, error: Error}> {
    ...
    try {
        roundData = await sql<RoundInfo>('rounds').select('rounds.*', 'courses.image', 'courses.name').where({playerOne: uid}).orWhere({playerTwo: uid}).orWhere({playerThree: uid}).orWhere({playerFour: uid}).innerJoin('courses', 'rounds.courseId', 'courses.id')
    } catch (e) {
        error = e;
    }
    return {
